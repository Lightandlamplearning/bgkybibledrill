...
function normalizeAnswer(ans) {
  return ans.toLowerCase().replace(/[^a-z0-9]/g, " ").replace(/\s+/g, " ").trim();
}
...
function TimerRing({ seconds, max }) {
  ...
  const color = seconds <= 2 ? "#FF6B6B" : seconds <= 4 ? "#FFC93C" : "#6BCB77";
  return (
    <svg width={72} height={72} viewBox="0 0 72 72">
      <circle cx={36} cy={36} r={r} fill="none" stroke="#e8e8e8" strokeWidth={5}/>
      <circle cx={36} cy={36} r={r} fill="none" stroke={color} strokeWidth={5}
        strokeDasharray={c} strokeDashoffset={c*(1-pct)}
        strokeLinecap="round" transform="rotate(-90 36 36)"
        style={{transition:"stroke-dashoffset 1s linear, stroke 0.3s"}}/>
      <text x={36} y={41} textAnchor="middle" fontSize={18} fontWeight="700" fill={color}>{seconds}</text>
    </svg>
  );
}
...
["distractor1", "distractor2", "distractor3"]`;
    ...
    try {
      // Use the Next.js API route so the Anthropic key stays server-side
  ...
  const handleAnswer = useCallback((userAnswer, isCorrect, fast = false) => {
    ...
    setShowResult({ correct: isCorrect, answer: q.answer_text_only, ref: q.bible_reference, original: q.original_answer });
  }, [questions, qIndex, streak, bestStreak, progress, missedIds, saveProgress, saveMissed]);
  ...
  const gameOptions = useMemo(() => {
    if (!currentQ) return [];
    const aiDistractors = aiDistractorCache[currentQ.db_id];
    const distractors = (aiDistractors && aiDistractors.length === 3)
      ? aiDistractors
      : getDistractors(currentQ, allQuestions);
    return shuffle([currentQ.answer_text_only, ...distractors]);
  }, [currentQ?.db_id, aiDistractorCache[currentQ?.db_id]]);
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"0 16px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:16, marginBottom:24 }}>
          {[
            ...
            { mode:"speed", icon:"⚡", label:"Speed Drill", desc:"7 seconds per question!", color:"#FFC93C" },
            ...
            <button key={m.mode} onClick={() => {
              ...
              style={{ background:"rgba(255,255,255,0.08)", border:`2px solid ${m.color}33`, borderRadius:16, padding:"20px 16px", cursor:"pointer", color:"#fff", textAlign:"left", transition:"all 0.2s", backdropFilter:"blur(10px)" }}
        ...
        <div style={{ background:"rgba(255,255,255,0.06)", borderRadius:16, padding:20, marginBottom:20, backdropFilter:"blur(10px)" }}>
...
<div style={{ maxWidth:600, width:"100%" }}>
          ...
          <div onClick={() => setStudyFlipped(f=>!f)} style={{ background:"rgba(255,255,255,0.08)", borderRadius:24, padding:"32px 24px", marginTop:8, cursor:"pointer", minHeight:200, display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"center", textAlign:"center", border:"2px solid rgba(255,255,255,0.1)", transition:"all 0.3s", backdropFilter:"blur(10px)" }}
          ...
          <div style={{ display:"flex", gap:12, marginTop:20, justifyContent:"center" }}>
            <button onClick={() => { setStudyIndex(i=>Math.max(0,i-1)); setStudyFlipped(false); }}
              ...
              style={{ flex:1, padding:"12px", background:studyIndex===0?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.12)", border:"none", borderRadius:12, color:studyIndex===0?"#555":"#fff", cursor:studyIndex===0?"not-allowed":"pointer", fontWeight:700, fontSize:16 }}>
              ← Prev
            </button>
            <button onClick={() => { setStudyFlipped(false); if(studyIndex+1>=questions.length){setScreen("studySetup");}else{setStudyIndex(i=>i+1);} }}
              style={{ flex:2, padding:"12px", background:"linear-gradient(90deg,#6BCB77,#4D96FF)", border:"none", borderRadius:12, color:"#fff", cursor:"pointer", fontWeight:800, fontSize:16 }}>
              {studyIndex+1 >= questions.length ? "Done! 🎉" : "Next →"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div style={{ maxWidth:650, margin:"0 auto" }}>
          ...
          {gameMode === "speed" && showResult === null && (
            <div style={{ display:"flex", justifyContent:"center", marginBottom:12 }}>
              <TimerRing seconds={timer} max={7}/>
            </div>
          )}

          <div style={{ background:"rgba(255,255,255,0.08)", borderRadius:20, padding:"24px 20px", marginBottom:16, backdropFilter:"blur(10px)", border:"1px solid rgba(255,255,255,0.1)" }}>
          ...
          {showResult === null ? (
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                    {options.map((opt,i) => (
                      <button key={i} onClick={() => {
                        ...
                        setSelectedOption(i);
                        handleAnswer(opt, isC, gameMode==="speed"&&timer>=5);
                      }}
            ...
            <div>
              <div style={{ background: showResult.correct?"rgba(107,203,119,0.15)":"rgba(255,107,107,0.15)", borderRadius:16, padding:"20px", border:`2px solid ${showResult.correct?"#6BCB7766":"#FF6B6B66"}`, marginBottom:16 }}>
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
                  <span style={{ fontSize:28 }}>{showResult.correct?"✅":"❌"}</span>
                  <span style={{ fontFamily:"'Fredoka One',cursive", fontSize:20, color:showResult.correct?"#6BCB77":"#FF6B6B" }}>
                    {showResult.correct?"Correct!":"Not quite..."}
                  </span>
                  {showResult.correct && gameMode==="speed"&&timer>=5 && (
                ...
                <div style={{ fontWeight:700, fontSize:16, color:"#fff" }}>Answer: {showResult.answer}</div>
          ...
          {showLeaderboard ? (
            ...
            <>
              ...
              {!nameSaved ? (
                ...
                <div style={{ background:"rgba(107,203,119,0.1)", border:"2px solid rgba(107,203,119,0.3)", borderRadius:16, padding:"12px 16px", marginBottom:16, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <span style={{ color:"#6BCB77", fontWeight:700 }}>✅ Score saved!</span>
                  <button onClick={() => setShowLeaderboard(true)}
              ...
              <div style={{ display:"flex", gap:10, marginBottom:10 }}>
                ...
                <button onClick={() => { setNameSaved(false); setPlayerName(""); const qs=shuffle(questions); setQuestions(qs); setQIndex(0); setScore(0); setStreak(0); setShowResult(null); setSelectedOption(null); setFreeInput(""); setScreen("game"); setTimer(7); }}
